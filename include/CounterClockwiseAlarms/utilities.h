...
namespace gr {
    namespace lora_sdr {
        ...
        /**
         *  \brief  Return an modulated downchirp using s_f=bw
         *
         *  \param  chirp
         *          The pointer to the modulated downchirp
         *  \param  id
         *          The number used to modulate the chirp
         * \param   sf
         *          The spreading factor to use
        ...
        inline void build_downchirp(gr_complex* chirp, uint32_t id, uint8_t sf, uint8_t os_factor = 1){
            ...
            for(uint n = 0; n < N* os_factor; n++){
                if(n<n_fold)
                    ...
                    chirp[n] = gr_complex(1.0,0.0)*gr_expj(-2.0*M_PI *(n*n/(2*N)/pow(os_factor,2)+(id/N+0.5)*(n-n_fold)/os_factor));
            }
        }
